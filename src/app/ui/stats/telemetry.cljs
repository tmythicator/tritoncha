...
(defn telemetry-component
  [{:keys [ctx-state clock-drift drift hardware-clock tone-now sample-rate
  ...
  (let [st      (or ctx-state "uninitialized")
    ...
    [:div.neo-section
     ...
     [:div.neo-grid
      ...
      [:div.neo-item [:span.neo-k "Base Latency: "] [:span.neo-v (if (and (number? lat) (pos? lat)) (str (.toFixed lat 2) " ms") "unavailable")]]
