...
(defn telemetry-component
  [{:keys [ctx-state clock-drift drift hardware-clock tone-now sample-rate lookahead base-latency base-lat transport-state transport-st bpm-str position pos key-str scene-name]}]
  (let [st       (or ctx-state "uninitialized")
    ...
    [:div.neo-section
     ...
     [:div.neo-grid
      ...
      [:div.neo-item [:span.neo-k "Base Latency: "] [:span.neo-v (if (and lat (pos? lat)) (str (.toFixed lat 2) " ms") "5.33 ms (ultra-low)")]]
