(ns app.ui.instrument-browser.state
  "State management and audition action bus for Instrument Studio."
  (:require
...
(defonce last-slider-preview-time (atom 0))
...
(defn inst-matches-category?
  ...
  (case cat-filter
    ...
    (= (sound-family inst-key spec) cat-filter)))
...
(def play-test-chord-or-arp!
  "Backwards-compatible alias for unified audition trigger.
  Examples: (play-test-chord-or-arp! :lead-blade) -> nil."
  play-test-chord!)
