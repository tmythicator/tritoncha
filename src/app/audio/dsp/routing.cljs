...
(defn- apply-bus-sends!
  ...
  [busses-spec]
  (let [target-sends (merge-with merge mixer/default-bus-sends busses-spec)]
    (doseq [[b-key s-map] target-sends]
      (mixer/set-send! b-key :delay (or (:delay s-map) 0.0))
      (mixer/set-send! b-key :reverb (or (:reverb s-map) 0.0)))))
...
(defn reset-fx!
  "Resets all DSP processors and bus sends back to the specification of the active routing topology.
  Examples: (reset-fx!)."
  []
  (set-routing! (or (:current-routing @audio-state) :default)))
